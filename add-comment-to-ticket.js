...
module.exports = function(RED) {
		...
		if(dot4ConfigNode){
			...
			node.on('input', async function(msg) {
				try{
					...
					node.status({fill:"blue",shape:"ring",text:"uploading comment data"});
					
					const createdComment = await incidentManagementApi.addTicketComment(msg.payload.ticketId, msg.payload.comment, msg.payload.type);

					msg.payload=createdComment
					node.send(msg);
					node.log(`add comment "${msg.payload.name}" in dot4`)
    ...
    RED.nodes.registerType("add-comment-to-ticket",addComment);
}